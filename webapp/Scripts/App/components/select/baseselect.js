 ...
 */
App.SelectCompComponent = Ember.Component.extend({
    tagName: "input",
    classNames: [""],
    classNameBindings: [],
    ...
    beforeDidInsertElement: function () { },
    didInsertElement: function () {
        this.beforeDidInsertElement();
        var self = this,
            options = {},
        ...
        options.id = (function (e) {
            return (e === undefined) ? null : Ember.get(e, optionIdPath);
        });
        ...
        options.formatResult = function (item) {
            ...
            var output,
                id = Ember.get(item, optionIdPath),
                text = Ember.get(item, optionLabelPath),
                headline = Ember.get(item, optionHeadlinePath),
        ...
        options.formatSelection = function (item) {
            ...
            var text = Ember.get(item, optionLabelSelectedPath || optionLabelPath);
                ...
                var filteredContent = self.get("content").reduce(function (results, item) {
                    ...
                    if (item.children) {
                        filteredChildren = item.children.reduce(function (children, child) {
                            if (select2.matcher(query.term, Ember.get(child, optionLabelPath)) || select2.matcher(query.term, Ember.get(child, optionHeadlinePath))) {
                                children.push(child);
                            }
                            return children;
                        }, []);
                    ...
                    if (select2.matcher(query.term, Ember.get(item, optionLabelPath)) || select2.matcher(query.term, Ember.get(item, optionHeadlinePath))) {
        ...
        this.set('compElement', this._select);

        this._select.on("change", Ember.run.bind(this, function () {
        ...
        this.addObserver('placeholder', this.placeholderChanged);
    ...
    willDestroyElement: function () {
        ...
        this.removeObserver('placeholder', this.placeholderChanged);
    },
    ...
    valueChanged: function () {
        ...
        while (!controller.isController) {
            controller = controller.get('targetObject');
        }

        if (self.get('changeAction')) {
            controller.send(self.get('changeAction'), value);
        }
    },

    placeholderChanged: function () {
        this.set('options.placeholder', this.get('placeholder'));
        this.$().select2(this.get('options'));
    },
